...
不過在解說之前，需要先強調不同程式語言的「非同步」概念，可能是不一樣的，而接下來只會著重在 JavaScript 的非同步概念上。

JavaScript 的非同步概念簡單來說可以這樣理解：<br />
是一種程式執行的方式，當程式執行到某個地方需要等待時，程式不會在這個地方等待執行完畢，而是會繼續往下執行。
:::
...
也就代表說，**會有一些東西其實是「不屬於 JavaScript 這個語言的」**。
...
1. JavaScript 只是程式語言，需要搭配執行環境提供的工具才可以做其他事，例如說  `setTiemout`、`document`  等等。
...
在現實世界中，正在執行的程式常常需要跟「外界」溝通並拿取或交換資料，然後對於正在執行的程式來說，外界就是「當前執行環境以外的執行環境」。
